...
## 2026-09-21 追記: 「表示」メニュー新設(右パネルタブの再表示機能、ユーザー要望)

右パネルのタブをフローティング(切り離し)状態にした後、×ボタンで閉じると、`CanClose="False"`の
ため実際には完全に削除されず「非表示(Hidden)」状態になるだけだが、それを再表示する手段がUI上に
無かった(実機確認で発覚)。ユーザーからの提案通り、上部メニューに新規「表示(_V)」メニューを追加し、
「表示 ＞ 右パネル ＞ (各タブ名)」というチェック付きメニュー構成で対応した。

### 実装
- `MainWindow.xaml`: 「共同編集」メニューの後に「表示(_V)」メニューを新設。中に`RightPanelViewMenu`
  (Header="右パネル(_R)")というサブメニューを1つ置き、`SubmenuOpened`イベントで項目を動的構築する
  (既存の「最近開いたファイル」メニュー(`RecentFilesMenu_SubmenuOpened`)と同じパターン)。
- `MainWindow.xaml.cs`: `RightPanelViewMenu_SubmenuOpened`を新設。`DockingManager.Layout.Descendents()`
  (ドッキング中・フローティング中のタブ)と`DockingManager.Layout.Hidden`(非表示中のタブ)の両方を
  合わせて全タブを洗い出し、タブごとにチェック付き`MenuItem`(`IsChecked`=`IsVisible`)を作る。クリックで
  `Hide()`/`Show()`を切り替える。固定10タブだけでなくプラグインパネル動的追加分も自動的に対象になる
  (`PropertyAnchorablePane`に限らず、レイアウト全体を`Descendents()`で走査するため)。

### 検証結果
クラウド側でビルド・publishレイアウト確認・テストスイート(695/703、既知の8件のみ)とも問題なし。実機での
挙動確認(フローティング→閉じる→「表示」メニューから再表示、の一連の流れ)は未実施のため、お願いしたい。

### 2026-09-21 追記: 「右パネル」メニューの並び順を固定化

実機確認(今回は実機テスト実施・目視確認済み)を経て、「表示メニューは表示状態に関わらず決まった順番で
並べたい」という要望を受けて対応。

**原因**: 当初の実装は`DockingManager.Layout.Descendents()`(ドッキング中・フローティング中のタブ)と
`DockingManager.Layout.Hidden`(非表示中のタブ)を都度走査してメニュー項目を作っていたが、この走査順は
タブがドッキング中か・フローティング中か・非表示中かによって変わりうるため、表示状態を切り替えるたびに
メニューの並びが変わってしまっていた。

**修正**: プラグインパネルを`PropertyAnchorablePane`へ追加する際、同時に`_pluginPanelAnchorablesリスト`
(新設フィールド)へも参照を記録するようにした。「右パネル」メニューを開く処理では、AvalonDock内部の走査を
一切使わず、固定10タブ(XAML宣言順の配列)→`_pluginPanelAnchorables`(追加順)という固定順で列挙する方式へ
変更した。表示状態(ドッキング/フローティング/非表示のいずれか)に関わらず、常に同じ並びになる。

### 検証結果
クラウド側でビルド・publishレイアウト確認とも問題なし。

### 段階的実装についての補足(ユーザーからの申し送り)
実機テストは毎回必ず挟むとは限らず、実機テストを飛ばして複数段階をまとめて進めることもある、との申し送りを
受けた。今回のAvalonDock導入(Step4)は実機テストを都度挟みながら進めたが、以後の段階(Step5含む)で
必ずしもそうとは限らない前提で進める。

### 次のステップ
右パネルの表示メニュー(並び順固定含む)の対応が完了したため、Step5(参加者一覧パネル)へ進む。

## 2026-09-21 追記: 参加者一覧パネル(5ステップ計画のStep5)

Step4で導入した右パネル(AvalonDock)の3つ目のペイン(`AnalysisAnchorablePane`)へ、「参加者一覧」タブを
追加した。共同編集セッション中の参加者(自分自身+他参加者)を、識別色の丸アイコン+表示名の一覧として表示する
(色はノート所有者アイコン(設計メモ6.4節)と同じ、`ParticipantInfo.Color`をそのまま使う)。

### 実装
- `CollabSessionController.cs`: 現在のロスターを外部から読み取れるよう、`public IReadOnlyList<ParticipantInfo>
  Roster => _roster;`を新設(既存の`RosterChanged`イベントは変化した瞬間のみ発火するため、パネルを最初に
  表示する時点や、接続直後でロスターがまだ0件(参加者は自分だけ)の場合に現在値を読み出す手段が無かった)。
- `MainWindow.xaml`: `AnalysisAnchorablePane`内、プレビュータブの後に`ParticipantsTabItem`
  (Title="参加者一覧")を追加。未接続時は案内文のみ、接続中は`ParticipantsListBox`(色付きアイコン+表示名の
  `ItemTemplate`)を表示する2段構成。
- `MainWindow.xaml.cs`:
  - `ParticipantRow(string Label, Brush ColorBrush)`という表示用レコードを新設。色は既存の`SafeColorBrush`
    ヘルパー(hexパース失敗時は灰色フォールバック)を再利用。
  - `RefreshParticipantsPanel()`を新設。`_collab`が`null`なら案内文のみ、そうでなければ`Self`(自分)を
    先頭に、`Roster`(自分以外)を続けて一覧化する。
  - `AttachCollabUiHandlers`: `collab.RosterChanged += _ => RefreshParticipantsPanel();`を追加。
    `Disconnected`ハンドラの中でも呼び、切断時に「未接続」表示へ戻す。
  - ホスト開始・参加(通常/仲介ヘルパー経由)いずれの接続成功パスでも、`_collab = collab;`の直後に明示的に
    `RefreshParticipantsPanel();`を呼ぶ(ホスト開始直後は他の参加者がまだ0人のため`RosterChanged`が
    発火せず、放置すると「未接続」表示のまま固まってしまうため)。
  - 「表示」メニュー(2026-09-21新設)の固定順配列にも`ParticipantsTabItem`を追加済み。

### 検証結果
クラウド側でビルド・publishレイアウト確認・テストスイート(695/703、既知の8件のみ)とも問題なし。実機での
動作確認(ホスト開始直後に自分自身が一覧に出るか、参加者の入退室で一覧が更新されるか、識別色がノート所有者
アイコンと一致しているか)は未実施のため、確認をお願いしたい。

### 5ステップ計画の状況
Step1〜5、いずれも実装完了(実機確認が済んでいないのはStep5の参加者一覧パネルの動作確認のみ)。
